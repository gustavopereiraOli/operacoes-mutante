...
function fatorial(n) {
  ...
  // Stryker disable next-line ConditionalExpression, LogicalOperator, EqualityOperator -- manter a condição de base evita mutantes equivalentes
  if (n === 0 || n === 1) return 1;
  let resultado = 1;
...
function produtoArray(numeros) {
  // Stryker disable next-line ConditionalExpression -- mutantes aqui resultam em comportamento idêntico
  if (numeros.length === 0) return 1;
  return numeros.reduce((acc, val) => acc * val, 1);
}
function clamp(valor, min, max) {
  // Stryker disable next-line EqualityOperator -- proteger contra mutantes equivalentes
